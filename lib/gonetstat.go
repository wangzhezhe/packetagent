...
func netstat(t string) []Process {
	...
	//get the data from the /proc/net/t file
	data := getData(t)
	...
	for _, line := range data {
		...
		//find the pid according the file description dir
		//fmt.Println(line_array[9])
		pid := findPid(line_array[9])
		exe := getProcessExe(pid)
		name := getProcessName(exe)
		fmt.Println(line_array[9], pid, exe, name)
...
func netstattcpforport(portip int) (Process, error) {
	// Return a array of Process with Name, Ip, Port, State .. etc
	// Require Root acess to get information about some processes.
	...
	return Process{}, errors.New("not find the pid acoording to specified portip")
}
...
func Tcpfromport(portip int) (Process, error) {
	// Get a slice of Process type with TCP and TCP6 data
	data, err := netstattcpforport(portip)
	if err != nil {
		return data, err
	}
	return data, nil
